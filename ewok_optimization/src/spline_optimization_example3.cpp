...
void depthImageCallback(const sensor_msgs::Image::ConstPtr& msg)
{
    ...
    dImg.convertTo(F32, CV_32FC1, 0.001);

    //std::cout << F32.size() << std::endl;
    ...
    */
    ...
    const float fx = 343.49636753580074;
    const float fy = 343.49636753580074;
    const float cx = 320.5;
    const float cy = 240.5;
	...
	//listener->lookupTransform("/map", "/camera_depth_optical_frame", msg->header.stamp, transform); 
    }
    ...
    for(int u=0; u < cv_ptr->image.cols; u+=4) {
	    ...
	    float val = data[v*cv_ptr->image.cols + u];
            //float val = data[v*cv_ptr->image.cols + u]/1000.0;

      //for(int u=0; u < F32.cols; u+=4) {
	  //for(int v=0; v < F32.rows; v+=4) {
	    //float val = data[v*F32.cols + u];
...
int main(int argc, char **argv) {
    ...
    //default
    //depth_image_sub_.subscribe(nh, "/camera/depth/image_rect_raw", 5);
    //tf::MessageFilter<sensor_msgs::Image> tf_filter_(depth_image_sub_, *listener, "/camera_depth_optical_frame", 5);

    //A Diep
    //depth_image_sub_.subscribe(nh, "/camera/ChangeDepth", 5);
    //tf::MessageFilter<sensor_msgs::Image> tf_filter_(depth_image_sub_, *listener, "/camera_depth_optical_frame", 5);
    ...
    //Eigen::Vector3d start_point(0.5, 0.0, 1.0), end_point(0.5, 0.0, 2.0);
    Eigen::Vector3d start_point(0.0, 0.5, 1.0), end_point(0.0, 0.5, 2.0);
    ...
    {
        start_reached = checkPosition(0.1, current_pose, targetTransfer(vec[0].x()-0.05, vec[0].y()-0.05, vec[0].z()));
...
void depthImageCallback(const sensor_msgs::Image::ConstPtr& msg)
{
    ...
    for(int u=0; u < cv_ptr->image.cols; u+=4) {
        for(int v=0; v < cv_ptr->image.rows; v+=4) {
            float val = data[v*cv_ptr->image.cols + u] / 1000;
...
int main(int argc, char **argv) {
    ...
    message_filters::Subscriber<sensor_msgs::Image> depth_image_sub_ ;
    depth_image_sub_.subscribe(nh, "/camera/depth/image_rect_raw", 5);
    tf::MessageFilter<sensor_msgs::Image> tf_filter_(depth_image_sub_, *listener, "/camera_depth_optical_frame", 5);  //camera_link
    tf_filter_.registerCallback(depthImageCallback);
    std::cout<< "2"<< std::endl;
    ...
    double distance_threshold_;
    pnh.param("distance_threshold", distance_threshold_, 1.2); //0.5
