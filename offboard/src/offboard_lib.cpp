...
void OffboardControl::plannerFlight()
{
    ...
    while (ros::ok())
    {
        ...
        // {
        //     curr_alpha_hover = calculateYawOffset(targetTransfer(hover_first.pose.position.x, hover_first.pose.position.x, hover_first.pose.position.x.z), setpoint);
        //     // target_enu_pose_.pose.orientation = tf::createQuaternionMsgFromYaw(curr_alpha_hover);
        ...
        //std::cout << "\nDelta yaw of curr_alpha_hover " << curr_alpha_hover << "\n";
        //components_vel_ = velComponentsCalc(vel_desired_, targetTransfer(current_odom_.pose.pose.position.x, current_odom_.pose.pose.position.y, current_odom_.pose.pose.position.z), setpoint);
        //target_enu_pose_ = targetTransfer(current_odom_.pose.pose.position.x, current_odom_.pose.pose.position.y, current_odom_.pose.pose.position.z, curr_alpha_hover);
    ...
    if (opt_point_received_)
    {
        ...
        while (ros::ok())
        {
            ...
            // {
            //     curr_alpha = calculateYawOffset(targetTransfer(current_odom_.pose.pose.position.x, current_odom_.pose.pose.position.y, current_odom_.pose.pose.position.z), targetTransfer(opt_point_.x, opt_point_.y, opt_point_.z));
            //     std::cout << "\n[Debug] delta yaw " << degreeOf(curr_alpha) << "\n";
            //     first_receive=false;
            // }
            // else
            // {
            //     curr_alpha = calculateYawOffset(targetTransfer(current_odom_.pose.pose.position.x, current_odom_.pose.pose.position.y, current_odom_.pose.pose.position.z), targetTransfer(opt_point_.x, opt_point_.y, opt_point_.z));
            //     std::cout << "\n[Debug] delta yaw_2222222222 " << degreeOf(curr_alpha) << "\n";
            // }
